...
class CoinTransactionAdmin(admin.ModelAdmin):
    ...
    list_display = ('id', 'user_display', 'amount_display', 'transaction_type', 'reason', 'created_at')
...
class CoinWithdrawalAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Withdrawal Information', {
            'fields': ('id', 'user_id', 'coins_amount', 'rupees_amount', 'upi_id'),
            'classes': ('wide',)
        }),
        ('Processing Status', {
    ...
    def upi_display(self, obj):
        """Display UPI ID professionally"""
        return format_html(
            '<div style="font-family: monospace; background: #e9ecef; padding: 4px 8px; border-radius: 4px; font-size: 12px;">'
            '<strong>{}</strong>'
            '</div>',
            obj.upi_id
        )
    upi_display.short_description = 'UPI ID'
